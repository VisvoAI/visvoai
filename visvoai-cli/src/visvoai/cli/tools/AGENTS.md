...
- `shellsafe.py` → `classify_command()` ('read'|'write' — decides PROMPTING only), `sandbox_argv()` (macOS sandbox-exec / Linux bwrap argv that denies file writes at the kernel), `looks_sandbox_denied()` (fallback-to-prompt heuristic). Consumed by `gated_tools.run_shell`.
...
# Gotchas
- `shellsafe` failure geometry: write-misclassified-as-read ⇒ no prompt BUT the OS sandbox blocks it (loud EPERM, no mutation, then falls back to the approval prompt); read-misclassified-as-write ⇒ one unnecessary prompt. The classifier decides prompting; the kernel is the boundary.
