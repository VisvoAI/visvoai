...
- `state.py` → per-project UI state at `~/.visvoai/projects/<pid>/state.json` (global, NOT in-repo). Tracks `last_seen_version` so the welcome banner can surface CHANGELOG entries since last visit. `get_state` is read-only (no side effects); `update_state` merges fields, skipping `None`, with atomic-write semantics (write to `.tmp`, `os.replace`).
