...
- `store.py` → folder-per-conversation persistence (`history.jsonl` + `meta.json` + `receipts.jsonl` + `checkpoints.jsonl` + per-branch `branches/<name>.jsonl`), no DB. `project_root(cwd)` is the stable shadow-repo work-tree anchor.
...
- `rewind.py` → `RewindMixin`: records checkpoints during a turn (best-effort, never breaks a turn); `/rewind` (Ctrl+B) restore-or-branch; `/branch` switch/fork timelines; `/fork` worktree + seeded conversation; `/export` transcript or bundle; `/log` the checkpoint chain. Checkpoints map message_index↔shadow commit so code + conversation move together. See `cli-git-structure`.
