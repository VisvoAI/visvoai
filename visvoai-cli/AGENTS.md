...
- `agent_turn.py` / `commands.py` / `sessions.py` / `render.py` / `demo.py` → VisvoApp mixins (real turn, slash commands, resume/screens, answer+mermaid rendering, the test-only mock). Turn failures render via `ErrorBlock` (classified auth/network/model by `_classify_turn_error`), not a SystemNote; an errored/interrupted turn still persists the human turn + the messages that completed (`_valid_thread_suffix` trims any dangling tool_call so the saved thread stays replayable).
