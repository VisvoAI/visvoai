...
the project. (They are also consumed by a private platform, which extends them
via subclassing + DI; that consumer is not described here.)

This repository is the source of truth for these packages. It was previously
generated from a private monorepo by `git subtree split`; that mirror is gone,
and contributions merge here.
...
- Each package ships a `README.md` (referenced by its `pyproject.toml`); the root `README.md` is this repository's landing page.
...
- Everything here is published, to GitHub and to PyPI. There is no private area of this repository.
