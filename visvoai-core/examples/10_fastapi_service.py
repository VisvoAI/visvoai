...
Production Caveats (What is NOT solved here):
  - Authentication & Rate Limiting: Omitted for clarity. In production, add auth
    middleware (e.g. OAuth2/JWT) and rate limiting (see issues #1 and #3).
...
def pick_model():
    ...
    class ScriptedModel(FakeMessagesListChatModel):
        _queue = []  # Shared across clones bound by tools
        ...
        async def ainvoke(self, messages, *args, **kwargs):
            ...
            return AIMessage(content="Task processed successfully.")
    ...
    ScriptedModel._queue = [
        ...
        AIMessage(content="Database status confirmed: healthy and operational."),
        AIMessage(content="Memory verified: thread state retained across requests."),
    ]
...
class ThreadStreamHub:
    ...
    def subscribe(self, thread_id: str) -> asyncio.Queue:
        ...
        # Replay recent buffer to new subscriber on reconnect
        for event_str in self._get_buffer(thread_id):
            q.put_nowait(event_str)
        return q
